...
import {
  ...
  TrendingUp, Activity, Shield, Menu, X, ChevronLeft, ChevronRight
} from "lucide-react";
...
export default function DashboardPage() {
    ...
    <>
      ...
      <div className="p-5 flex items-center justify-between" style={{ borderBottom: "1px solid var(--border)" }}>
        <Link href="/" className="flex items-center gap-2" onClick={() => setSideOpen(false)}>
          <ShieldCheck className="text-blue-500 flex-shrink-0" size={22} />
          {!isCollapsed && <span className="text-lg font-bold gradient-text-blue">FairLens</span>}
        </Link>
        {!isCollapsed && (
          <button onClick={() => setCollapsed(true)} className="hidden md:block p-1 rounded-lg transition-colors hover:bg-white/10"
            style={{ color: "var(--text-muted)" }} title="Collapse sidebar">
            <ChevronLeft size={16} />
          </button>
        )}
        {isCollapsed && (
          <button onClick={() => setCollapsed(false)} className="hidden md:block p-1 rounded-lg transition-colors hover:bg-white/10"
            style={{ color: "var(--text-muted)" }} title="Expand sidebar">
            <ChevronRight size={16} />
          </button>
        )}
      </div>
      {!isCollapsed && <p className="text-xs px-5 pt-2" style={{ color: "var(--text-muted)" }}>AI Bias Auditor</p>}

      {/* Nav */}
      <nav className="flex-1 p-4 space-y-1">
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : ''} gap-3 px-3 py-2.5 rounded-xl text-blue-400 text-sm font-medium`}
          style={{ background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.2)" }}
          title={isCollapsed ? "Dashboard" : ""}>
          <BarChart3 size={17} />{!isCollapsed && " Dashboard"}
        </div>
        <Link href="/audit/new" onClick={() => setSideOpen(false)}
          className={`flex items-center ${isCollapsed ? 'justify-center' : ''} gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors hover:bg-white/5`}
          style={{ color: "var(--text-secondary)" }} title={isCollapsed ? "New Audit" : ""}>
          <Plus size={17} />{!isCollapsed && " New Audit"}
        </Link>
        <Link href="/demo" onClick={() => setSideOpen(false)}
          className={`flex items-center ${isCollapsed ? 'justify-center' : ''} gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors hover:bg-white/5`}
          style={{ color: "var(--text-secondary)" }} title={isCollapsed ? "Live Demo" : ""}>
          <Zap size={17} />{!isCollapsed && " Live Demo"}
        </Link>
      </nav>
      ...
      <div className="p-4" style={{ borderTop: "1px solid var(--border)" }}>
        ...
        {isCollapsed && (
          <div className="flex justify-center mb-3">
            <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm text-blue-400"
              style={{ background: "rgba(59,130,246,0.15)" }} title={user?.name || "User"}>
              {user?.name?.[0]?.toUpperCase() || "U"}
            </div>
          </div>
        )}
        <button onClick={logout}
          className={`w-full flex items-center ${isCollapsed ? 'justify-center' : ''} gap-2 px-3 py-2 rounded-lg text-sm transition-colors hover:bg-red-500/10 hover:text-red-400`}
          style={{ color: "var(--text-muted)" }} title={isCollapsed ? "Sign Out" : ""}>
          <LogOut size={15} />{!isCollapsed && " Sign Out"}
        </button>
      </div>
    </>
  );
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg-page)" }}>
      ...
      <aside className={`hidden md:flex fixed left-0 top-0 bottom-0 ${collapsed ? 'w-[72px]' : 'w-64'} flex-col z-40 transition-all duration-300`}
      ...
      {/* ── Main Content ─────────────────────────────────────────────────────── */}
      <main className={`${collapsed ? 'md:ml-[72px]' : 'md:ml-64'} p-4 md:p-8 transition-all duration-300`}>
